public class Axis{
  ...
  private final color COLOR_TITLE_DEFAULT  = color(0, 0, 0);
  private final color COLOR_TITLE_SELECTED = color(255, 0, 0);
  private final color COLOR_RANGE_DEFAULT  = color(0, 0, 255, 60);
  private final color COLOR_RANGE_SELECTED = color(255, 0, 0, 60);
  ...
  private Viewport minKnob;
  private Viewport mainKnob;

  public Axis(String title, String comment,float max, float min, float startX, float startY, float endX, float endY){
    this.title = title;
  ...
  private void drawTitleWith(color backgroudColor){
    fill(backgroudColor);
    textAlign(LEFT, TOP);
    text(this.title, this.titleView.getX(), this.titleView.getY() - 10);
  }
  private void drawAxis(){
    ...
    text(nf(this.max, 0, 2), this.startX - this.range.getWidth() + 20, this.startY - 20); //max
    text(nf(this.min, 0, 2), this.endX - this.range.getWidth() + 20, this.endY + 20); //min
  }
